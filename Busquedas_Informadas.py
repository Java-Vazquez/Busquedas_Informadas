...
Original file is located at
    https://colab.research.google.com/drive/1Hlbff0UKF4bRe23SFoXjt2TJFDK4m5O7
"""
...
Para poder usar el sistema si se corre en google colab solo debes copiar y pegar el código y después presionar ejecutar celda.
En caso de querer correrlo desde visual studio u alguna otra herramienta, se requiere la extensión de matlib ya que para el
simulated annealing se uso esa librería para poder generar la imagen del grafo original contra la versión nueva.
Dicho esto procede a pasar lo mismo, se abre el código y se presiona ejecutar código python.

"""

"""
Ya en ambos casos lo siguiente es lo mismo, se cuenta con un menú de 12 opciones:
1.  Greedy best-first
2.  A* con peso
3.  A*
4.  Beam
5.  Branch and Bound
6.  Steepest hil climbing
7.  Stochastic hil clambing
8.  Simulated annealing
9.  Genetic Algorithm
10. Todos los anteriores
11. Mostrar grafo procesado
12. Salir del programa

Después de este menú, se tiene un segundo submenú para las opciones del 1-9.
Este segundo menú nos permitirá hacer la opción de si queremos tener pasos o no.
Al seleccionar si queremos pasos o no lo único que va a ocurrir es que ocurriran los
procesos de diferente manera. Después de que ocurra este proceso se vuelve a
preguntar por que proceso quieres hacer y cuando se seleccione se vuelve a preguntar
con o sin pasos.
Tras terminar este proceso, si ya no queremos continuar dentro del sistema se usa
la opción 12 y en automático se sale del proceso.
...
"""
En este primer código se hará el simulated annealing con pasos y aplicando el uso
de hormigas en este caso serían las iteraciones que son las veces que el código
se va a ejecutar para generar el resultado final ya con la ruta más óptima
o la ruta más corta
"""
#comenzamos definiendo el nombre con variables que se le van a pasar como el valor
#de estado inicial, el destino, la lista de adyacencias que viene más adelante
#el número máximo de veces que queremos repetir el códifo (iteraciones) y nuestra
#tempratura que pues sería la inicial que nos permitirá ir reduciendo hasta quedar
#lo más cerca de 0

def simulated_annealing_con_pasos(initial_state, destination, adjacency_list, max_iterations, temperature): #Francisco Anaya Viveros
  # El current state es el estado inicial
    current_state = initial_state
  # El costo actual se hace en base a la heuristica el estado actual
    current_cost = heuristic(current_state)
  # El mejor estado como va iniciando pues es el estado acual que va a ir cambiando
  # confforme se repita el código
    best_state = current_state
  # El mejor costo sucede algo parecido, va a estar almacenado en la variable costo
  # actual que va a ir incrementando ya que se van a ir sumando los demás costos.
    best_cost = current_cost
  
  #Empieza a generar el bucle a partir del número de iteraciones que se le pida
  #como por ejemplo si se le dice que sean 100 pues el bucle lo va a repetir
  #hasta llegar a ese valor
    for i in range(max_iterations):
        # Generar un vecino aleatorio a partir del estado actual y la lista de adyacencias
        # porque no podemos pedirle que de un nodo haga un vecino con otro estado con el
        # que no exista un camino
        neighbors = get_neighbors(current_state, adjacency_list)
        # El siguiente estado sirve para guardar el nuevo vecino generado del paso anterior
        next_state = random.choice(neighbors)
        # El próximo costo se calcula a partir del nuevo estado o siguiente estado
        # del paso anterior, este dato se manda a la heuristica para sacar el peso
        # del estado anterior al nuevo y así después sumarlo
        next_cost = heuristic(next_state)

        # Mostrar información sobre la iteración
        #Nos va a decir el número de iteración empezando por el 0 hasta la que se le pidiera
        #pero para que se vea bien la cantidad por eso es i + 1 así sería de la 1 hasta la 
        #iteracón pedida
        print(f'Iteración {i+1}:')
        #En temperatura actual es el cálculo de la iteración, con el porcentaje de reducción
        #con la temperatura inicial
        print(f'    Temperatura actual: {temperature}')
        # El estado actual te va a decir en que nodo o en este caso estado se encuentra más
        # el costo hasta el momento que sería la suma de todos los costos que se han ido
        # generando
        print(f'    Estado actual: {current_state} (costo: {current_cost})')
        #El vecino generado pues es lo último antes de comenzar a imprimir que genera un
        #vecino a partir de las conexiones que tenga el estado y calcula su costo (next_cost)
        #que se mostrará y sabemos que se sumará después al costo actual
        print(f'    Vecino generado: {next_state} (costo: {next_cost})')

        # Evaluar si se acepta el vecino
        # Se checa que el nuevo vecino contra los demás que se pudieran tener para
        # tener el de menor costo de ahí que el próximo costo se le reste el costo
        # actual, si este es mejor que 0 entonces el estado actual se convierte
        # en ese nuevo valor y el costo actual se le sumará el nuevo costo y nos
        # dirá que es la mejor opción encontrada
        delta_cost = next_cost - current_cost
        if delta_cost < 0:
            current_state = next_state
            current_cost = next_cost
            print('    Aceptado: Mejora encontrada')
        else:
           # En caso de que esto no suceda se checará la probabilidad del mismo valor
           # pero dividido por la temperatura a ver cual pudiera ser el camino con la
           # menor probabilidad de ser el peor.
            prob = math.exp(-delta_cost / temperature)
            if random.random() < prob:
                current_state = next_state
                current_cost = next_cost
                print('    Aceptado: Peor solución aceptada con probabilidad {:.2f}%'.format(prob*100))
            else:
                print('    Rechazado: No se acepta peor solución')

        # Actualizar la mejor solución encontrada
        # En esta sección lo único que vamos a hacer es comparar si el costo actual
        # es menor que el mejor costo, en caso de serlo entonces el mejor estado se
        # debe volver el estado actual en el que estemos y el mejor costo debería
        # ser entonces el costo actual.
        if current_cost < best_cost:
            best_state = current_state
            best_cost = current_cost
            #Al final imprimos que la nueva mejor solución es con un costo que son los
            #que tenemos arriba
            print(f'    Nueva mejor solución encontrada: {best_state} (costo: {best_cost})')

        # Actualizar la temperatura
        # El porcentaje que queremos que reduzca con cada iteración
        temperature *= 0.9999
    # Aquí regresamos estas dos variables que serán usadas después.
    return best_state, best_cost

def get_neighbors(node, adjacency_list): #Francisco Anaya Viveros
# Este código sirve para los mencionados antes que nos ayudan a encontrar un vecino
# por eso recibe el nodo y la lista de adyacencias, al final lo único que regresa
# es el nodo ubicado en la lista de adyacencias para poder saber quienes si tienen
# conexión.
    return adjacency_list[node]

def dibujar_mapa(): #Francisco Anaya Viveros
# Comenzamos con las latitudes y longitudes de cada punto
  latitudes = {'Cancún': 21.1213285, 'Valladolid': 20.688114, 'Felipe Carrillo Puerto': 19.5778903, 'Campeche': 19.8305682, 'Merida': 20.9800512, 
                 'Ciudad del Carmen': 18.6118375, 'Chetumal': 18.5221567, 'Villa Hermosa': 17.9925264, 'Tuxtla': 16.7459857, 'Francisco Escarcega': 18.6061556, 
                 'Acayucan': 17.951096, 'Tehuantepec': 16.320636, 'Alvarado': 18.7760455, 'Oaxaca': 17.0812951, 'Puerto Angel': 15.6679974, 
                 'Izucar de Matamoros': 18.5980563, 'Tehuacan': 18.462191, 'Pinotepa Nacional': 16.3442895, 'Cuernavaca': 18.9318685, 
  ...
  longitudes = {'Cancún': -86.9192738, 'Valladolid': -88.2204456, 'Felipe Carrillo Puerto': -88.0630853, 'Campeche': -90.5798365, 'Merida': -89.7029587, 
    ...
    #En esta línea vamos a definir cuantas figuras vamos a querer tener juntas y sus tamaños, la primer medida del figsize es el alto y el segundo el ancho
  fig, ax = plt.subplots(1, 2, figsize=(40,15)) # especificar el tamaño de la figura
  ...
  ax[0].set_title("Grafo antes de la búsqueda", fontsize=25) # establecer el título del primer subplot
    ...
    # Realizar búsqueda
  best_state, best_cost = simulated_annealing_con_pasos(start_verificado, goal_verificado, adjacency_list, max_iterations, temperature)
...
#Este segundo proceso hace lo mismo que el anterior pero en este caso omitimos el escribir todos los pasos de cada iteración
# y que nos diga en que nuevo nodo se encuentra, etc... ya que lo único que queremos en este segundo es solo hacer el cálculo
# y que nos regrese el resultado sin pasar por todos los pasos.
